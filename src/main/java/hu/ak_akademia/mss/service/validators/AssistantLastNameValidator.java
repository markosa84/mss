...
class AssistantLastNameValidator implements Validator<Assistant> {
    ...
    public void validate(Assistant assistant) throws IncorrectEnteredDataException {
        if (assistant.getLastName() == null || !assistant.getLastName().matches(LAST_NAME_REGEX)) {
            throw new IncorrectEnteredDataException("lastNameError", "Incorrect! Correct form is e.g.: Teszt");
        }
        MSSUserValidatorFactory.lengthValidation(assistant.getLastName());
    }
